...
export const CellAction = ({ data }: CellActionProps) => {
  ...
  const onUpdate = () => {
    navRouter.push(`/${storeId}/sizes/${sizeId}`);
  };

  const onDelete = () => {
    startTransition(async () => {
      try {
        const deleteSizeRes = await axios.delete(
          `/api/${storeId}/sizes/${sizeId}`
        );

        if (deleteSizeRes.status != 200) {
          console.log(deleteSizeRes);
          throw new Error("Something went wrong!");
        }

        const responseData = deleteSizeRes.data;
